    ...
    simply.field = Backbone.View.extend({
        ...
        validate: function(silent) {
            ...
            if(this.options.validation.length === 0) {
                ...
                for(var x=0; x< this.options.validation.length; x+=1) {
                    if(typeof this.options.validation[x].type === 'function') {
                        //evaluate the func
                        if(this.options.validation[x].type.apply(this)) {
                            validCount +=1;
                        }
                    }else{
                        //Switch through some defaults
                        switch (this.options.validation[x].type) {
                            case "required":
